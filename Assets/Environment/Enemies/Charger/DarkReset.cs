	...
	void Update () 
	{
		...
		if (canHurt && (transform.localScale.x / 2) + 2.0f > distanceBetween)
		{
			...
			if (knockBack)
			{
				...
				Vector3 knockBackV = (player.transform.position - transform.position).normalized * knockBackAmount;
				//Vector3 knockBackV = GetComponent<DarkCharger>().dirToPlayer.normalized * knockBackAmount;
